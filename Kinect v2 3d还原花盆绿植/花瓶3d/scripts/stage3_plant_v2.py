"""
Stage 3 v2: Plant — frame 0 as seed, incremental ICP to accumulated model
"""
import numpy as np
import cv2
...
# ============================================================
# Extract plant clouds per frame (camera coordinates)
# ============================================================
print("Extracting plant clouds per frame...")
plant_pcds = []  # in camera coords, with normals
for i in range(N):
    depth=load_depth(i)
    ...
    if len(plant_pts) < 30:
        print(f"  Frame {i:03d}: too few pts ({len(plant_pts)}), skipping")
...
# ============================================================
# Incremental ICP: frame 0 as seed, each frame registers to accumulated model
# ============================================================
print("\nIncremental ICP (frame 0 = seed, each frame -> accumulated model)...")

# Frame 0 = seed (in camera coords, identity pose = world)
seed = plant_pcds[0]
world_cloud = o3d.geometry.PointCloud(seed)  # accumulated model in world coords
world_poses = [np.eye(4)]  # camera->world transform per frame

for i in range(1, N):
    src = plant_pcds[i]
    if src is None:
        world_poses.append(None)
        print(f"  Frame {i:03d}: skip")
        continue

    # Initial: undo turntable rotation to bring close to world (frame 0 coords)
    init = rot_center(-i * DEG)

    # ICP to accumulated model
    try:
    ...
    src_world = o3d.geometry.PointCloud(src)
    src_world.transform(T)

    # Add to accumulated model
    world_cloud += src_world
    # Periodic downsample to control size
    if i % 3 == 0:
        world_cloud = world_cloud.voxel_down_sample(0.002)

    world_poses.append(T)
    print(f"  Frame {i:03d}: fit={fit:.3f} rmse={rmse:.4f}m | model={len(world_cloud.points)} pts")
...
# Visualize
# ============================================================
print("\nPLANT COLORED CLOUD (ICP-aligned) — close to see mesh.")
o3d.visualization.draw_geometries(
    [world_cloud], window_name="Stage 3 — Plant ICP-Aligned Cloud",
    width=1280, height=720)

print("PLANT MESH — close window.")
o3d.visualization.draw_geometries(
    [mesh], window_name="Stage 3 — Plant Mesh (ICP)",
...
print("POT + PLANT (ICP-aligned) — close window to finish Stage 3.")
o3d.visualization.draw_geometries(
    [combined], window_name="Stage 3 — Pot + Plant (ICP)",
    width=1280, height=720, mesh_show_back_face=True)
